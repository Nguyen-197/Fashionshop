...
type IPasswordControlProps = IFieldProps & {
}
// https://formik.org/docs/examples/dependent-fields
const PasswordControl = forwardRef((props: IPasswordControlProps, ref) => {
    ...
    return (
        <>
            <div>
                {props.rateValue? <div className='row'>
                    ...
                    {props.suffixesContent && <span className="my-suffix">{props.suffixesContent}</span>}
                        <input
                            ...
                            ref={textboxRef}
                            className={`${getStyleClass('password')} ${props.disabled ? 'disable' : ''} ${isFormFieldValid(props.fieldPath) ? 'p-invalid' : ''} ${props.textAlign ? `text-${props.textAlign}`: ''} ${props.meter ? `p-password-meter`: ''}`}
                            value={value}
                            type="password"
                ...
                </div> : <div className="p-input-icon-right">
                    {props.suffixesIcon && <i className={`pi ${props.suffixesIcon}`}/>}
                    {props.suffixesContent && <span className="my-suffix">{props.suffixesContent}</span>}
                    <input
                        name={props.fieldName}
