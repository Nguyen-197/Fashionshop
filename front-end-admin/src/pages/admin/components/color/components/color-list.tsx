...
type IColorListProps = StateProps & DispatchProps & {
    onAdd?: () => void, // Thêm mới
    onEdit?: (rowData) => void, // Cập nhật
    onDelete?: (rowData) => void, // Xóa
}

const ColorList = (props: IColorListProps) => {
    ...
    const onPage = (event) => {
        ...
        props.handleSearchColor(props.formSearch, eventConfig);
    }
        ...
        <>
            <div className='wrap-table'>
                <Panel headerTemplate={templateHeader}>
                    <DataTable value={props.listColor?.data} totalRecords={props.listColor?.recordTotal * 1} first={props.listColor?.first * 1} rows={10} lazy
                    ...
                    currentPageReportTemplate={translate('common.currentPageReportTemplate')} paginator={props.listColor?.data?.length > 0} selectionMode="single"
                    emptyMessage={translate('common.dataNotFound')} sortField={configTable.sortField} sortOrder={configTable.sortOrder} onSort={onSort} onPage={onPage}>
                        ...
                        <Column field="code" header={translate('color.code')} sortable></Column>
                        <Column field="name" header={translate('color.name')} sortable></Column>
                        <Column header={translate('common.actionLabel')} body={renderActionColumn} style={{ width: '100px' }}></Column>
                    </DataTable>
                </Panel>
            </div>
        </>
...
const mapStateToProps = ({ colorReducerState, locale, authentication }: IRootState) => ({
    currentLocale: locale.currentLocale,
    listColor: colorReducerState.listColor as any,
