...
const CustomerForm = forwardRef((props: ICustomerFormProps, ref: any) => {
    ...
    const {
        ...
        validationSchema: yup.object().shape({
            ...
            email: yup.string().max(200).required(),
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            if (props.userId) {
                ...
                const formData = {};
                try {
                    const restCus = await UserService.findById(props.userId);
                    if (restCus.data.type == RESPONSE_TYPE.SUCCESS) {
                        const restCusData = restCus.data.data;
                        formData['fullName'] = restCusData.fullName;
                        formData['email'] =  restCusData.email;
                        formData['phoneNumber'] =  restCusData.phoneNumber;
                        formData['active'] =  restCusData.active;
                    }
                } catch (error) {
                    console.log(">>>> error: ", error);
                }
                try {
                    const restAddress = await AddressService.findByUserId(props.userId);
                    if (restAddress.data.type == RESPONSE_TYPE.SUCCESS) {
                        const restCusData = restAddress.data.data;
                        formData['address']['province'] = restCusData.province * 1;
                        formData['address']['district'] = restCusData.district * 1;
                        formData['address']['ward'] = restCusData.ward;
                        formData['addressDetail'] = restCusData.addressDetail;
                    }
                } catch (error) {
                    console.log(">>>> error: ", error);
                }
    ...
    const onSubmit = async (data) => {
        ...
        await CommonUtil.confirmSaveOrUpdate(async () => {
            ...
            await UserService.saveOrUpdate(data).then(async res => {
                if (res.data.type == RESPONSE_TYPE.SUCCESS) {
                    const formAddress = {
                        ...
                        isDefault: true,
                        addressFull: provinceAddress + " " + districtAddress + " " + wardAddress
                    }
                    const rest = await AddressService.saveOrUpdate(formAddress);
                    if (res.data.type == RESPONSE_TYPE.SUCCESS) {
                        setDisplayBasic(false);
                        props.afterSaveSuccess && props.afterSaveSuccess(formAddress.userId);
                    }
                }
            }).catch(error => {
