        ...
        <>
            <div className='wrap-table'>
                <Panel headerTemplate={templateHeader}>
                    <DataTable value={props?.listUser?.data} totalRecords={props?.listUser?.recordTotal * 1} first={props.listUser?.first * 1} rows={10} lazy selection={selectedCustomers} onSelectionChange={e => setSelectedCustomers(e.value)}
                        ...
                        <Column field="role.name" header="Quyền hạn" body={roleBodyTemplate}></Column>
                        <Column field="active" header="Trạng thái" body={activeBodyTemplate}></Column>
                        <Column header="Thao tác" align="center" alignHeader="center" body={renderActionColumn} style={{ width: '100px' }}></Column>
                    </DataTable>
                </Panel>
            </div>
        </>
...
const mapStateToProps = ({ locale, userReducerState, authentication }: IRootState) => ({
    currentLocale: locale.currentLocale,
    listUser: userReducerState.listUser as any,
    formSearch: userReducerState.formSearch,
